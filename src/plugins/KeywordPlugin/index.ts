...
function KeywordPlugin(): JSX.Element {
  ...
  const createHashtagNode = react.useCallback((textNode: LexicalNode) => {
    return hashtag.$createHashtagNode(textNode.getTextContent());
  }, []);
  const getHashtagMatch = react.useCallback((text: string) => {
